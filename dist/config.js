...
export function readConfigFromPath(configPath) {
    let content;
    try {
        content = readFileSync(configPath, 'utf-8');
    }
    catch (err) {
        throw new Error(`Could not read config from ${configPath}. Run the i18n-mcp setup skill to create one.`, { cause: err });
    }
    let raw;
    try {
        ...
        throw new Error(`Invalid JSON in ${configPath}: ${err.message}`, { cause: err });
    }
